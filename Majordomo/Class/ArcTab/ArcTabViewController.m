...
- (void)dealloc {
  ...
  [[NSNotificationCenter defaultCenter] removeObserver:self name:kNArcTabToggleTabBar object:nil];
  [super dealloc];
}
...
tabBarBackgroundColor:(UIColor *)tabBarBackgroundColor
  ...
  if (self = [self init]) {
    ...
    CGRect tabBarFrame =
      (CGRect){{(kArcTabViewWidth - tabBarSize_.width) / 2.f, CGRectGetHeight(self.viewFrame)}, tabBarSize_};
    // Generate tab bar
    tabBar_ = [[ArcTab alloc] initWithFrame:tabBarFrame
...
- (void)viewDidLoad {
  ...
  for (int i = 0; i < [self.tabBarItems count]; ++i) {
    UIViewController *viewController = [[self.tabBarItems objectAtIndex:i] objectForKey:@"viewController"];
    ...
    if ([viewController isKindOfClass:[UINavigationController class]]) {
      ...
      viewHeight = vc.navigationBar.frame.size.height;
    }
    [view.layer setPosition:CGPointMake(view.frame.size.width / 2, kArcTabViewHeight + viewHeight / 2)];
  }
  
  // Notification for togglling tab bar
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(toggleTabBar:)
                                               name:kNArcTabToggleTabBar
                                             object:nil];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
  ...
  UIView * currentView = [self.view viewWithTag:kNArcTabSelectedViewControllerTag];
...
- (void)touchDownAtItemAtIndex:(NSUInteger)itemIndex
  ...
  [viewController.view setTag:kNArcTabSelectedViewControllerTag];
